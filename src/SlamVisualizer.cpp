...
void SlamVisualizer::initDraw()
{
    ...
    s_cam_ = pangolin::OpenGlRenderState(
        pangolin::ProjectionMatrix(WIN_WIDTH_, WIN_HEIGHT_, 210, 210, 320, 240, 0.1, 1000),
        pangolin::ModelViewLookAt(-1, 1, 5.5, 0, 0, 0, pangolin::AxisZ)
    );
    ...
    imageTexture_ = pangolin::GlTexture(640, 480, GL_RGB, false, 0, GL_BGR, GL_UNSIGNED_BYTE);
    // 跟踪图片显示
    ...
    trackTexture_ = pangolin::GlTexture(640, 480, GL_RGB, false, 0, GL_BGR, GL_UNSIGNED_BYTE);
}
...
void SlamVisualizer::drawPointCloud(std::vector<Eigen::Vector3d> &pts_3d, std::vector<Eigen::Vector3d> &rgb)
{
    glPointSize(2.0);
    glBegin(GL_POINTS);
    for (size_t i = 0; i < pts_3d.size(); i++)
    {
        glColor3f(rgb[i](0)/255.0, rgb[i](1)/255.0, rgb[i](2)/255.0);           // 修改颜色
        glVertex3f(pts_3d[i](0), pts_3d[i](1), pts_3d[i](2)); // 设置定点坐标
    }
    glEnd(); // 结束
}

void SlamVisualizer::drawTrajGt(vector<Eigen::Vector3d> &traj){
    if(!traj_visible_)
        return;
    glLineWidth(2);
    glBegin(GL_LINES);
